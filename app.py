...
def update_secret(api:str, store:dict):
    match api:
        case 'v1':
            for password in ['POSTGRES_PASSWORD','KEA_DB_PASSWORD','PDNS_DB_PASSWORD']:
        ...
        case 'v2-devel' | 'v2':
            ...
            for password in ['dhcp_db_password','dns_db_password','admin_db_password',]:
...
def server(args):
    ...
    def errorhandler(e):
        return render_template_string(page_template, embed=welcome_embed, errortext='<p><b>Error 404<b></p>'), 404
...
def main():
    ...
    parser_server = subparsers.add_parser('server', description='Run app.py as a server. Takes no arguments.')
    parser_server.set_defaults(func=server)

    parser_client = subparsers.add_parser('client')
    parser_client.set_defaults(func=client)
    parser_client.add_argument('input',
                               metavar='inputYaml',
                               action='store',
                               help='YAML file to read vars from')
    parser_client.add_argument('filename',
                               metavar='templateFile',
                               action='store',
                               help='name of file to render from template dir')
    parser_client.add_argument('-a', '--api',
                               action='store',
                               default='v1',
